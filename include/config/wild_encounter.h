...
#define WE_OWE_RESTRICT_METATILE                FALSE   // If TRUE, OWEs will stay within tiles with the same encounter metatile behavior as the one it is currently on, if any.
#define WE_OWE_RESTRICT_MAP                     FALSE   // If TRUE, OWEs will stay within their current map bounds.
...
#define WE_OWE_SPAWN_REPLACEMENT                TRUE    // If TRUE, the oldest OWE objects will despawn after a short time and be replaced with a new spawn if possible.
...
#define WE_OWE_PREVENT_SPECIAL_MOVEMENT_DESPAWN FALSE   // If TRUE, shiny OWEs (if WE_OWE_PREVENT_SHINY_DESPAWN is TRUE) or Feebas OWEs (if WE_OWE_PREVENT_FEEBAS_DESPAWN is TRUE) will not be despawned by their movement type (for example, OWE_DESPAWN_ON_NOTICE).
#define WE_OWE_DESPAWN_ON_ENTER_TOWN            FALSE   // If TRUE, despawns all OWEs upon entering a city (MAP_TYPE_CITY) or town (MAP_TYPE_TOWN).
